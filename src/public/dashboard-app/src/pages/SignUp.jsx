...
const signupFeatures = [
  {
    ...
    desc: 'Encrypted storage for all your API credentials and access tokens.',
  },
  {
    icon: '🔗',
    title: 'Multi-Service Integration',
    desc: 'Connect GitHub, Google, Slack, Discord, and many more services.',
  },
  {
    icon: '⚡',
    title: 'Quick Setup',
    desc: 'Get started in minutes. No complex configuration required.',
  },
];
...
function SignUp() {
  ...
  useEffect(() => {
    ...
    if (callback) {
      if (callback.status === 'connected') {
        // OAuth successful, fetch user and redirect to onboarding
          ...
          .then((sessionUser) => {
            if (sessionUser) {
              ...
              if (sessionUser?.isFirstLogin) {
                ...
                window.history.replaceState({}, document.title, '/dashboard/onboarding');
                setTimeout(() => {
  ...
  const handleOAuthClick = async (serviceId) => {
    setError('');
    const params = new URLSearchParams({ mode: 'signup', returnTo: '/dashboard/signup', redirect: '1' });
        ...
        <div className="grid gap-8 lg:grid-cols-12 lg:gap-12 xl:gap-16">
            ...
            <div className="max-w-xl">
              <div className="mb-8">
                ...
                <p className="mt-4 text-base text-slate-300">
                  Join thousands of developers who are simplifying their API management and automation workflows.
                </p>
              </div>
              ...
              <div className="mt-10 rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
                <p className="text-sm text-slate-300">
                  <span className="font-semibold text-blue-300">Free tier included</span> — Start building immediately with our free plan. Upgrade anytime.
                </p>
              </div>
            </div>
          </section>
          ...
          <section className="lg:col-span-7">
            <div className="rounded-3xl border border-slate-700/80 bg-slate-900/85 p-5 shadow-2xl shadow-black/40 sm:p-7 lg:p-8">
              <div className="mb-6">
                <h2 className="text-2xl font-semibold">Sign up with your account</h2>
                <p className="mt-2 text-sm text-slate-400">Choose your preferred authentication method to get started.</p>
              </div>
              ...
              <div className="mt-6 border-t border-slate-700 pt-6">
                <p className="text-center text-xs text-slate-400 mb-4">
                  Already have an account? <a href="/dashboard/login" className="font-semibold text-blue-400 hover:text-blue-300 transition-colors">Sign in</a>
                </p>
              </div>
