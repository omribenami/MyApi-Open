...
function EditPersonaModal({ onSuccess }) {
  ...
  useEffect(() => {
    if (showEditModal && selectedPersona) {
      setFormData({
        name: selectedPersona.name || '',
        description: selectedPersona.description || '',
        vibe: selectedPersona.vibe || '',
        emoji: selectedPersona.emoji || '🤖',
        soul_content: selectedPersona.soul_content || '',
        traits: Array.isArray(selectedPersona.traits) ? [...selectedPersona.traits] : [],
      });
      setTraitsInput('');
      setLocalError(null);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!selectedPersona) {
      setLocalError('No persona selected');
      return;
    }
    ...
    try {
      ...
      const success = await onSuccess(selectedPersona.id, submitData);
      ...
      setLocalError(err.message || 'Failed to update persona');
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="bg-slate-800 rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-96 overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex gap-3 pt-4 border-t border-slate-700">
            ...
            >
              {saving ? 'Updating...' : 'Update Persona'}
            </button>
          </div>
        </form>
      </div>
